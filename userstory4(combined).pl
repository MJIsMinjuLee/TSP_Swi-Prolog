...
%dataTruck_t_e_ta(<truckName>,<start_city>,<end_city>,<time>,<energy>,<aditional_time>).

carateristicasCam(eTruck01,7500,4300,80,100,60).
%truckCharacteristics(<type>,<tare>,<maxLoad>,<batteryCapacity>,<autonomy>,<batChargeTime>)

%The different deliveries, add one by one
...
% making a list of the cities with deliveries 
citiesDelivery(Xs):-
    findall(X,(delivery(_,_,_,X,_,_)),Xs).
    
% finding all possible routes adding start and end point
possible_deliveries(AD):-
    start_city(SP),
    citiesDelivery(Xs),
    findall(LF,(permutation(Xs,D),append([SP|D],[SP],LF)),AD).
    
% finding the times from one warehouse to another
timeToCities(C1,C2,TTime):- 
    dadosCam_t_e_ta(_,C1,C2,Time,_,Atime),
    TTime is Time + Atime.
...
% finding the total load of the deliveries.
totalLoad(C1,Tot):-
    findall(L,(delivery(_,_,L,C,_,_),C1 is C),Ls),
    sum_list(Ls,Tot).

% finding the time between two city and arranging in descending order
min([Item],Item).
min([Item|List],Item):-
...
calculation_combined(C1,C2,R):-
    dadosCam_t_e_ta(_,C1,C2,T,_,_),
    delivery(_,_,LW,C2,_,_),
    citiesDelivery(Xs),
    cities(Xm),
    member(C2,Xs),
    member(C1,Xm),
...
next_city(Current,Next,Cities):-
    min_combined(Current,Cities,ListofCombined),
    min(ListofCombined,TimeandLoad),
    member(Next,Cities),
    calculation_combined(Current,Next,TimeandLoad).

min_combined(Current,[City | Cities],[L | Tail]):-
    calculation_combined(Current,City,L),
    min_combined(Current,Cities,Tail).
min_combined(_,[],[]).

finallist(AddEnd,HeuristicsCombined):-
    start_city(SP),
    tsp(HeuristicsList,HeuristicsCombined),
    last(HeuristicsList,LastElement),
    append(HeuristicsList,[SP],AddEnd).

tsp([SP | Paths],Combinations):-
    start_city(SP),
    citiesDelivery(Cities),
    tsp(SP,Cities,Paths,Combinations).

tsp(_,[],[],0).
tsp(Current,Remaining,[Next | Path],TotalCombination):-
    next_city(Current,Next,Remaining),
    calculation_combined(Current,Next,D),
    delete(Remaining,Next,NewRemaining),
    tsp(Next,NewRemaining,Path,D1),
    TotalCombination is D + D1.

%finds the real time and energy consumtions between two cities
calculate_energytime(REnergy,RTime,C1,C2):-
    totalLoad(_,Load),
    carateristicasCam(_,EmptyTuck,MaxLoad,_,_,_),
    dadosCam_t_e_ta(_,C1,C2,Time,Energy,_),
    RTime is Time*(EmptyTuck+Load)/(EmptyTuck+MaxLoad),
    REnergy is Energy*(EmptyTuck+Load)/(EmptyTuck+MaxLoad), EL is 80-REnergy.

%finds the current energy of the truck
currentEnergy(CEnergy):-
    calculate_energytime(REnergy,_), 
    currentEnergy(CE1),
    CEnergy is CE1-REnergy.

%finds the chargeTime
chargeTime(CTime):-
    currentEnergy(CEnergy),
    carateristicasCam(_,EmptyTuck,MaxLoad,BCapacity,Autonomy,BatChargeTime),
    dadosCam_t_e_ta(TruckName,C1,C2,Time,Energy,Atime),
    CTime is (0.8*BCapacity-CEnergy)*BatChargeTime/48.

%calculates the time for each route including charging
calculate_time1(Xcomp,Time, LCharging):- 
    finallist(Xcomp,_),
    carateristicasCam(_,EmptyTuck,MaxLoad,BCapacity,Autonomy,BatChargeTime),
    routeTime1(Xcomp,Autonomy,Time,LCharging).

%iterates to through list of cities and finds total time, including where to charge  
routeTime1([_],_,0,[]).
routeTime1([C1,C2|Xs], Autonomy,TTime,LCharging):- 
    dadosCam_t_e_ta(_,C1,C2,Time,Energy,Atime), calculate_energytime(REnergy,RTime,C1,C2),
    ((Autonomy<REnergy,!,carateristicasCam(_,_,_,_,A,TCharge), 
    A1 is A-REnergy, 
    LCharging = [C1|LPreviousCharges]);
    A1 is Autonomy-REnergy, 
    TCharge is 0,
    LCharging = LPreviousCharges),
    routeTime1([C2|Xs],A1, Time1,LPreviousCharges),
    TTime is Time1+Atime+Time+TCharge.


%chooses the shortest route 
shortestTimeSeq(Xs,LCharging,Time):-
    (run;true),
    time_min(Xs,LCharging,Time).

run:-
    retractall(time_min(_,_,_)), 
    assertz(time_min(_,_,100000)),
    finallist(Xs,_),
    calculate_time1(Xs,Time,LCharging),
    update(Xs,Time,LCharging),
    fail.

update(Xs,Time,LCharging):-
    time_min(_,_,MinTime),
    ((Time<MinTime,!,
    retract(time_min(_,_,_)),
    assertz(time_min(Xs,LCharging,Time)),
