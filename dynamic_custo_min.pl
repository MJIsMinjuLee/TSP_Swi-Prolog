...
%id_warehouse(<local_warehouse>,<id>).
%truck_feature(<truck_name>,<truck_weight>,<truck_capacity>,
<truck_total_battery>,<autonomia?>,<t_recarr_bat_20a80?>).
%dataTruck_t_e_ta(<truckName>,<start_city>,<end_city>,<time>,
<energy>,<aditional_time>).
%delivery(<deliveryId>,<date>,<deliveryLoad>,<deliveryWarehouse>,
<loadTime>,<loadPickupTime>).

%making a list of the cities with deliveries
citiesDelivery(Date1,Xs):-
    findall(X,(entrega(_,Date,_,X,_,_),Date1 is Date),Xs).
...
%sum of the loads
sum_weight([],[],0).
sum_weight([City|LC],[WeightAc|LP],WeightAc):-
    sum_weight(LC,LP,WeightAc1),
    entrega(_,Date,Weight,City,_,_),
    WeightAc is Weight+WeightAc1.

%adding truck weight to the loads
add_truck_weight(Truck,[],[]).
add_truck_weight(Truck,[Weight|LP],[TruckWeight|LPT]):-
    add_truck_weight(Truck,LP,LPT),
    carateristicasCam(_,Truck,_,_,_,_),
    TruckWeight is Weight+Truck.

%making random list of loads and truck weight
random_list_add_truck(Date,L1):-
    Date1 is Date,
    find_all(Date1,DeliveryList),
    add_ends(DeliveryList,LScomplete),
    sum_weight(DeliveryList,SumWeight,FirstWeight),
    add_truck_weight(Truck,SumWeight,AddTruck),
    carateristicasCam(TruckId,TruckWeight,TruckCapacity,TruckBattery,A,X),
    append(AddTruck,[TruckWeight],L1).
...
%getting fraction for calculation
calculate_weight(Date,LA,LB):-
    random_list_add_truck(Date,LA),
    calculate_weight(Date,[W|L1],[H|L2]).
calculate_weight(Date,[],_):-!.
calculate_weight(Date,[W|L1],[H|L2]):-
    carateristicasCam(TruckId,TruckWeight,TruckCapacity,TruckBattery,A,X),
    round(W/(TruckWeight + TruckCapacity),H,2),
    calculate_weight(Date,L1,L2).

%calculating total time of delivery
calculate_all_time(Date,LScomplete,Timee):-
    find_all(Date,DeliveryList),
    add_ends(DeliveryList,LScomplete),
    all_time(LScomplete,Timee).
all_time([LV],[]).
all_time([C1,C2|CL],[T1|Ttime1]):-
    (dadosCam_t_e_ta(TruckId,C1,C2,T1,Energy,AddTime)),
    all_time([C2|CL],Ttime1).
