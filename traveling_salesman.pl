%utvide dimensjonen av problemet(flere varehus som skal innom) og verifiser til hviken grad det er gjennomførbart å fortsette på brukt metode (med en generator av alle mulige løsninger) ved å gjennomføre en studie av kompleksiteten av problemet
...
%aevaluerer veiene i henhold til tiden det tar å fullføre alle leveringene og returnere til Matosinhos base varehus og velg veien som er raskest
...
%makes a list of the cities with deliveries 
citiesDelivery(Date1,Xs):-
    findall(X,(delivery(_,Date,_,X,_,_), Date1 is Date),Xs).

%Adds start_city to the list 
%add_ends(L,LF1):- 
    %start_city(SP), append([SP|L],[SP], LF).
%finds all possible routes
possible_deliveries(AD):- 
    citiesDelivery(_,Xs), findall(D,permutation(Xs,D),AD).

%finds the times from one warehouse to another
timeToCities(C1,C2,TTime):- 
    (dadosCam_t_e_ta(_, C1,C2,Time,_,Atime)), TTime is Time+Atime.
...
%finds the total load of the deliveries. Works
totalLoad(C1,Tot):-
    findall(L,(delivery(_,_,L,C,_,_),C1 is C),Ls), sum_list(Ls,Tot).


%finds the real time and energy consumtions between two cities. Works
...
calculate_time1(Xs,Time, LCharging,Xcomp):- 
    citiesDelivery(_,Xs),
    start_city(SP),
    append([SP|Xs],[SP],Xcomp),
    carateristicasCam(_,EmptyTuck,MaxLoad,BCapacity,Autonomy,BatChargeTime),
...
shortestTimeSeq(Xs,Time):-
    (run;true), 
    time_min(Xs,Time).

run:-
    retractall(time_min(_,_)), 
    assertz(time_min(_,100000)),
    citiesDelivery(_,Xs),
    permutation(Xs,Xperm),
    calculate_time(Xperm,Time),
    update(Xperm,Time),
    fail.
...
update(Xperm,Time):-
    time_min(_,MinTime),
    ((Time<MinTime,!,
    retract(time_min(_,_)),
    assertz(time_min(Xperm,Time)),nl);
    true).
