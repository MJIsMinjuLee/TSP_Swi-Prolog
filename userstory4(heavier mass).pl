...
next_city(Current,Next,Cities):-
    max_loadweight(Current,Cities,ListofLoads),
    max(ListofLoads,MaxLoad),
    member(Next,Cities),
    weightandtime(Current,Next,MaxLoad).

max_loadweight(Current,[City | Cities],[L | Tail]):-
    weightandtime(Current,City,L),
    max_loadweight(Current,Cities,Tail).
max_loadweight(_,[],[]).

finallist(AddEnd,HeuristicsWeight):-
    start_city(SP),
    tsp(HeuristicsList,HeuristicsWeight),
...
tsp([SP | Paths],Loads):-
    ...
    tsp(SP,Cities,Paths,Loads).

tsp(_,[],[],0).
tsp(Current,Remaining,[Next | Path],TotalWeight):-
    next_city(Current,Next,Remaining),
    weightandtime(Current,Next,D),
