...
% finding the time between two city and arranging in ascending order
min([Item],Item).
min([Item|List],Item):-
...
next_city(Current,Next,Cities):-
    min_distance(Current,Cities,NextD),
    min(NextD,TD),
    member(Next,Cities),
    distance(Current,Next,TD).

min_distance(Current,[City | Cities],[D | Tail]):-
    distance(Current,City,D),
    min_distance(Current,Cities,Tail).
min_distance(_,[],[]).

finallist(AddEnd,TotalTime):-
    start_city(SP),
    tsp(HeuristicsList,HeuristicsTime),
    last(HeuristicsList,LastElement),
    distance(LastElement,SP,T),
    append(HeuristicsList,[SP],AddEnd),
    TotalTime is HeuristicsTime + T.

tsp([SP | Paths],Distances):-
    start_city(SP),
    citiesDelivery(Cities),
    tsp(SP,Cities,Paths,Distances).

tsp(_,[],[],0).
tsp(Current,Remaining,[Next | Path],Distance):-
